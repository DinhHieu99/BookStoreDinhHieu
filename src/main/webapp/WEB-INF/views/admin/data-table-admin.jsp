										...
										<tbody>
											<c:forEach var="admin" items="${admin}">
												<tr>
													...
													<td>${admin.mobile}</td>
													<td>${admin.email}</td>
													<td><c:if test="${admin.status == true}">
															<span class="badge-dot badge-success mr-1"></span>Online
														</c:if> <c:if test="${admin.status == false}">
															<span class="badge-dot badge-secondary mr-1"></span>Offline
														</c:if></td>
														
													<td><c:if test="${admin.status_login == true}">
															<span class="badge-dot badge-success mr-1"></span>Cho phép đăng nhập
														</c:if> <c:if test="${admin.status_login == false}">
															<span class="badge-dot badge-secondary mr-1"></span>Không cho phép đăng nhập
														</c:if></td>
														
													<td><c:if test="${admin.position == true}">
															<span class="badge-dot badge-success mr-1"></span>Quản lý
														</c:if> <c:if test="${admin.position == false}">
															<span class="badge-dot badge-secondary mr-1"></span>Nhân viên
														</c:if></td>
													<td>
														<div class="btn-group ml-auto">
															<a href="/admin/edit-admin/${admin.id}"><button
																	class="btn btn-sm btn-outline-light">Edit</button></a> <a
																href="/admin/remove-admin/${admin.id}"><button
